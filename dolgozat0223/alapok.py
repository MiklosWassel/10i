...
vnev = input("Kérem a vezték neved: ")
...
def addSzor(a,b):
    return (a+b)*2
print(f"Eredmeny: 2,3 -> {addSzor(2,3)}")
'''
#5
versenyzok = list()
...
for sor in fajl:
    #print(sor.replace("\n","").split(";"))
    bontott = sor.replace("\n","").split(";")
    szotar = {"Helyezes":int(bontott[0]),"Nev":bontott[1],"Orszag":bontott[2],"Nyeremeny":int(bontott[3])}
    #print(szotar)
...
print(f"A világranglistán {len(versenyzok)} versenyző szerepel")
...
index = 0
while versenyzok[index]["Helyzes"] != 1:
    index +=1
print(v) 